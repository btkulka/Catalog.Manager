...
namespace ManagerUI.Controllers
{
    public class ComponentsController : CMController
    {
        ...
        public ActionResult Components_Deactivate(int? compId)
        {
            try
            {
                var comp = db.Components.Where(w => w.ComponentId == compId && (bool)w.IsActive).FirstOrDefault();
                comp.IsActive = false;
        ...
        public ActionResult Components_RestoreDeactivated(int? compId)
        {
            try
            {
                ...
                var comp = db.Components.Where(w => w.ComponentId == compId).OrderByDescending(s => s.CreatedDate).FirstOrDefault();
                comp.IsActive = true;
